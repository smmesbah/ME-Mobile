...
const SelectTimeDrawer: React.FC<SelectTimeDrawerProps> = ({
  ...
  useEffect(() => {
    if (startTime !== undefined) {
      setIsSpecifiedTimeEnable(true);
      const timeZoneOffsetInHours = moment().utcOffset() / 60;
      const time=moment(startTime).add(timeZoneOffsetInHours, "hours").toDate()
      setStartTimeDummy(new Date(time));
      Animated.parallel([
    ...
    if (endTime !== undefined) {
      // setIsSpecifiedTimeEnable(true);
      const timeZoneOffsetInHours = moment().utcOffset() / 60;
      const time=moment(endTime).add(timeZoneOffsetInHours, "hours").toDate()
      setEndTimeDummy(new Date(time));
      setIsPointTime(false);
    }
  }, []);
  ...
  const handleTimeSave = () => {
    ...
    if (isPointTime) {
      ...
      setEndTime(
        endTimeDummy === null
          ? moment(startTimeDummy)
              .add(timeZoneOffsetInHours * 60 + 1, "minutes")
              .toDate()
        ...
        <View style={[styles.bottomSheet]}>
          ...
          <Text style={[styles.text2]}>
            {!isSpecifiedTimeEnable
              ...
              : isPointTime
              ? `Do it at ${moment(startTimeDummy).format(
                  "hh:mm A"
                )} of the day`
              : `Do it between ${moment(startTimeDummy).format(
                  "hh:mm A"
                )} and ${
                  endTimeDummy === null
                    ? moment(startTimeDummy).add(1, "minute").format("hh:mm A")
                    : moment(endTimeDummy).format("hh:mm A")
                } of the day`}
          </Text>
            ...
            {!isPointTime && (
              <View style={[styles.container10]}>
                ...
                <DateTimePicker
                  ...
                  value={
                    endTimeDummy === null
                      ? moment(startTimeDummy).add(1, "minutes").toDate()
                      : endTimeDummy
                  }
                  mode="time"
                  is24Hour={true}
                  minimumDate={moment(startTimeDummy)
                    .add(1, "minutes")
                    .toDate()}
